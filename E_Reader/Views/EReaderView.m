...
- (void)showCursor{
    ...
    [self removeCursor];
    _leftCursor = [[ECursorView alloc] initWithType:CursorLeft andHeight:self.font byDrawColor:[UIColor blueColor]];
    _rightCursor = [[ECursorView alloc] initWithType:CursorRight andHeight:self.font byDrawColor:[UIColor blueColor]];
...
- (void)LongPressAction:(UILongPressGestureRecognizer *)longPress{
    ...
    if (longPress.state == UIGestureRecognizerStateBegan ||
        ...
        [_delegate eReaderView:self shutOffGesture:YES];
        [_delegate eReaderView:self hideSettingToolBar:nil];
...
- (void)ciBa:(id)sender{
    ...
    NSString *encodedString = [ciBaString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(eReaderView:ciBa:)]) {
        [self.delegate eReaderView:self ciBa:encodedString];
    }
    [self resetting];
}
...
- (void)copyword:(id)sender{
    ...
    [_delegate eReaderView:self shutOffGesture:NO];
...
- (EMagnifiterView *)magnifierView {
    
    if (_magnifierView == nil) {
        _magnifierView = [[EMagnifiterView alloc] init];
        _magnifierView.backgroundColor = [[EDataSourceManager shareInstance] getThemeImage];
