...
@implementation ESettingBottomBar
{
    ILSlider *ilSlider;
    UILabel  *showLbl;
    BOOL isFirstShow;
...
- (void)configUI{
    ...
    [ilSlider sliderTouchEndBlock:^(CGFloat value) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(settingBottomBar:sliderToChapterPage:)]) {
            [_delegate settingBottomBar:self sliderToChapterPage:page];
...
- (void)themeButtonPressed:(UIButton *)sender{
    ...
    [EDataCenter saveCurrentThemeID:sender.tag-7000];
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(settingBottomBar:themeButtonAction:themeIndex:)]) {
        [_delegate settingBottomBar:self themeButtonAction:self themeIndex:sender.tag-7000];
...
- (void)goToNextChapter{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(settingBottomBar:turnToNextChapter:)]) {
        [self.delegate settingBottomBar:self turnToNextChapter:nil];
    }
}

- (void)goToPreChapter{
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(settingBottomBar:turnToPreChapter:)]) {
        [self.delegate settingBottomBar:self turnToPreChapter:nil];
    }
...
- (void)changeSmall{
    ...
    NSUInteger fontSize = [EDataCenter e_getFontSize];
    if (fontSize <= MIN_FONT_SIZE) {
//        [E_HUDView showMsg:MIN_TIPS inView:self];
        return;
    }
    fontSize--;
    [EDataCenter saveFontSize:fontSize];
    [self updateFontButtons];
    if (self.delegate && [self.delegate respondsToSelector:@selector(settingBottomBar:fontSizeChanged:)]) {
        [self.delegate settingBottomBar:self fontSizeChanged:(int)fontSize];
    }
...
- (void)showDrawerView{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(settingBottomBar:callDrawerView:)]) {
        [self.delegate settingBottomBar:self callDrawerView:nil];
    }
...
- (void)showCommentView{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(settingBottomBar:callCommentView:)]) {
        [self.delegate settingBottomBar:self callCommentView:nil];
    }
}
